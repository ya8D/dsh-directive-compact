...
- **Implementation status.** The command, planning, and summarization implementation (P1–P5) is merged; bundle install and with-key e2e against the real DeepSeek API are verified. The AI-driven free-trim command (P6) is in progress; the compact-directive polish (P7: required directive, shrink validation) is planned. The package is not yet published to the npm registry, so installs must come from a local checkout or tarball.
