...
- **`/trim-directive` trims the whole surface, with no system-node protection (P10)**: the trim range is now the ENTIRE surface — the head, the injected `agent-instructions` / `system-prompt` / `skill-catalog` skeleton, and any prior `compact` checkpoint are all trim-able like any other node (rc.0 protected them). This is safe because the agent loop re-injects the skeleton on every request; only tool-pairing balance remains as a session-integrity constraint. The fix was driven by "精确删除测试2": the old "start after the last injected node" rule misfired on forked/compacted sessions, where automatic compaction folds the original skeleton into a checkpoint and the skeleton re-injects mid-conversation.
