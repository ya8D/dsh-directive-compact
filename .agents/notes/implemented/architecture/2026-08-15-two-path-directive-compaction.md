...
**The head includes the session's fixed skeleton, not merely the first message.** A real-session survey (14+ sessions, including a fresh one-turn session) established that every DeepSeek Harness session opens with the user's first message followed by the `agent-instructions` injection, the `system-prompt` snapshot, and the `skill-catalog` listing. These injected nodes are the surface records of per-request injections — the agent loop re-injects the skeleton on every request (source-verified in P10) — so the head is a conservative task anchor: the first user message and the early user turns carry the task, and the skeleton records stay stable between requests. Folding the head into the summarized span loses that anchor. After a prior compaction, a `compact` checkpoint precedes them and is absorbed into the skeleton (it is a non-`user`-sourced `user/message`).
