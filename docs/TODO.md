...
## P13.1 — Harness compliance review notes (user review 2026-08-16)

User review of the P13 standardization against the harness package rules. The harness rules do not bind this plugin repository; these items are recorded for a maintainer decision. The three flagged points, in priority order:

- [ ] **Hardcoded tunables are not Config fields (most important; harness AGENTS.md "No hardcoded tunables in plugins")**: `REWRITE_INFLATION_RATIO = 1.1` (`src/op-mode.ts`) and `TRIM_CHUNK_INPUT_BUDGET = 50_000` / `TRIM_MAX_CHUNKS = 20` (`src/trim.ts`) are deployment-varying thresholds exposed neither in the `Config` schema nor in the README `## Configuration` table. The existing five Config fields (keepHeadUsers / keepTailUsers / summarizationProvider / summarizationModel / maxTokens) comply; only these three constants do not. If adopted: promote them to Config fields (e.g. `rewriteInflationRatio` default 1.1, `trimChunkInputBudget` default 50_000, `trimMaxChunks` default 20) and thread them through `executeTrim` / `validateOpManifest`. Do it BEFORE the first stable release — after that, exposing a new field is not breaking, but changing the hardcoded value is; before release it is free. (The user confirmed this is a record, not a mandate: harness rules do not bind this repo.) **Config-lockstep**: the hardcoded `20 × 50K` figures in the README Known Limitations and `docs/model-experience.md` (Token effect) must update together with the Config defaults if this is adopted.
- [x] **README `## Model Experience` `/trim-directive` over-detail (done in this round)**: the entry described internal mechanisms (FORM 1 / FORM 2 labels, prose reuse, fallback dispatch) that belong in the Agent Note (`2026-08-16-operation-mode-trim.md` owns the rationale). Trimmed to the user-visible contract: what the model sees (numbered rendering + manifest syntax) with a link to the Operation-mode section for execution semantics.
- [x] **README states the typical trim wall time (done in this round)**: "A typical trim takes around three minutes on a small conversation; larger surfaces take longer (see Known Limitations)" added to the `/trim-directive` Usage note; the Known Limitations time note stays.
- [x] **README split into a user manual (done in this round)**: developer-facing sections moved out — Model Experience → `docs/model-experience.md` (full harness-format contract), Observability → `docs/observability.md` (log lines, debug level, slow-run diagnosis), Development → link to `docs/testing.md`. Dropped as duplicates or non-user content: Extension points (AGENTS.md / src JSDoc own them), the four Deferred items (TODO Future owns them), "No cross-chunk aggregate output cap" and "No before/after rendering in UI" limitations (mechanism detail / duplicate of the Where-the-command section), pure-increment merged into one Install sentence. README is now the user-facing manual (~103 lines): commands, install, usage, manifest syntax, no-change behavior, UI behavior, configuration, user-facing limitations, further-reading links.

Non-blocking observation (user suggestion, not flagged):

- [ ] **AGENTS.md references the harness prose/doc skills**: the plugin AGENTS.md cites the harness Agent-Note rules and testing tiers but not `dsh-prose-standard` / `dsh-doc-standards`. Since the README now follows the harness doc structure, add a one-line reference in AGENTS.md so future doc edits have a norm anchor.
